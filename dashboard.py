...
def schedule_auto_off(hours: float):
    ...
    st.success(f"Device will auto turn off after {hours} hour(s) at {st.session_state.scheduled_off_time.strftime('%H:%M:%S')}")
...
with col_on:
    if st.button("🔋 Turn ON", type="primary"):
        toggle_device(True)
        st.session_state.power_on = True

with col_off:
    if st.button("⛔ Turn OFF"):
        toggle_device(False)
        st.session_state.power_on = False
...
if st.session_state.scheduled_off_time:
    st.info(f"Scheduled OFF time: {st.session_state.scheduled_off_time.strftime('%Y-%m-%d %H:%M:%S')}")
    if datetime.now() >= st.session_state.scheduled_off_time:
        toggle_device(False)
        st.session_state.scheduled_off_time = None
        st.session_state.auto_off_active = False
...
# ------------------- Trend -------------------
# ------------------- Energy and Cost over time -------------------
st.subheader("📊 Energy & Cost Over Time")

fig1 = go.Figure()

fig1.add_trace(go.Scatter(
    x=df['Time'], y=df['Energy (kWh)'],
    name="Energy (kWh)",
    mode='lines+markers',
    line=dict(shape='spline', color='limegreen', width=3),
    marker=dict(size=6)
))

fig1.add_trace(go.Scatter(
    x=df['Time'], y=df['Cost (BDT)'],
    name="Cost (BDT)",
    mode='lines+markers',
...
st.subheader("📉 Full Metric Trend: Current, Voltage, Power, Energy, Cost")
fig2 = go.Figure()
fig2.add_trace(go.Scatter(x=df['Time'], y=df['Current (mA)'], mode='lines', name='Current (mA)', line=dict(color='cyan')))
fig2.add_trace(go.Scatter(x=df['Time'], y=df['Voltage (V)'], mode='lines', name='Voltage (V)', line=dict(color='orange')))
fig2.add_trace(go.Scatter(x=df['Time'], y=df['Power (W)'], mode='lines', name='Power (W)', line=dict(color='blue')))
fig2.add_trace(go.Scatter(x=df['Time'], y=df['Energy (kWh)'], mode='lines+markers', name='Energy (kWh)', line=dict(color='green')))
fig2.add_trace(go.Scatter(x=df['Time'], y=df['Cost (BDT)'], mode='lines', name='Cost (BDT)', line=dict(color='red')))
...
st.caption("🔧 Developed by Momena Khatun Zinia | Styled for CSE407 Green Computing")
